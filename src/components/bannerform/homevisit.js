    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.status === "success") {
          ...
          let useAlternative =
            res.data.data.alternateResult.length === 0 ? false : true;
          let alternativeResult = useAlternative
            ? res.data.data.alternateResult
            : [];

          dispatch(
            updateResult(
              res.data.data.professional,
              useAlternative,
              alternativeResult,
              res.data.data.groupPractice
            )
          );
        ...
        <Row gutter={0}>
          ...
          <Col lg={9} xs={24}>
            <Form.Item
              ...
              // ]}
            >
              <Select placeholder="Location" allowClear showSearch>
                {locations.map((item) => (
