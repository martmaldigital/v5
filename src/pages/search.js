...
const Search = () => {
  ...
  useEffect(() => {
    if (search !== undefined && search.searchObj !== null) {
      ...
      setLocationChoosen(
        search.visitType === "Video Consultation"
          ? "Video Consultation"
          : search.searchObj.location
      );
    }
  }, [search]);
  ...
  const checkVisitTypeChange = (value) => {
    ...
    axios
      .get(`${api}/api/professional/check-id/${currentDoctor.id}`)
      .then((res) => {
  ...
  const triggerChange = (e) => {
    ...
    if (visitTypeChoosen === null) {
      ...
      if (
        ...
        axios
          .get(
            `${api}/api/professional/${currentDoctor.id}/sessions/${day}/${location}/${visitTypeChoosen}`
          )
          .then((res) => {
